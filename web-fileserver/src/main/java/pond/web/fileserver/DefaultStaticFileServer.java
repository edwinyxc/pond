...
public class DefaultStaticFileServer implements StaticFileServer {
    ...
    public DefaultStaticFileServer(String dir) {
        S._assert(dir);
        this.dir = dir;

        String _root = PATH.isAbsolute(dir)
                           ? dir
                           : S.config.get(NetServer.class,
            NetServer.CONFIG_WEB_ROOT
        ).concat(File.separator).concat(dir);
    ...
    public void apply(HttpCtx http) {
        ...
        String absPath = sanitizeUri(path, ctx.currentRoute());
        ...
        if (file.isHidden() || !file.exists()) {
            //do not handle since this is a toCtxHandler
            response.send(404 );
            return;
        }
